...
const btnGhost: React.CSSProperties = {
  ...
  border: `1px solid ${theme.border}`,
  background: "#fff", color: theme.textSecondary,
...
export function EstateList() {
  ...
  return (
    <div style={{ animation: "fadeInUp 0.3s ease" }}>
      ...
      {/* Create form */}
      {creating && (
        <GlassCard title="Open New Estate" subtitle="Register a deceased TNG account for probate processing" style={{ marginBottom: 20 }} accent>
          <form onSubmit={handleCreate}>
            ...
            {error && (
              <div style={{
                ...
                background: theme.errorLight, color: "#991B1B",
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              {loading ? (
                  ...
                  <tr key={i}>{[160,100,120,60,90,60].map((w, j) => (
                    <td key={j} style={{ padding: "14px 16px" }}>
                      <div style={{
                        ...
                        backgroundSize: "400% 100%", animation: "shimmer 1.4s ease-in-out infinite",
                      }} />
                    </td>
                  ))}</tr>
                ))
                ...
                <tr
                  key={e.id}
                  style={{ transition: "background 0.1s", borderBottom: `1px solid ${theme.borderLight}` }}
                ...
                >
                  ...
                  <td style={{ padding: "14px 16px", fontSize: 13, fontFamily: theme.fontMono, color: theme.textSecondary }}>
                  ...
                  <td style={{ padding: "14px 16px", fontSize: 12, color: theme.textMuted }}>
                    {new Date(e.created_at).toLocaleDateString("en-MY", { day: "2-digit", month: "short", year: "numeric" })}
                  </td>
                  <td style={{ padding: "14px 16px" }}>
                    <Link to={`/estates/${e.id}`} style={{
