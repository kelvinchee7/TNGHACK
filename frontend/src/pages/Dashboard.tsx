...
function KpiCard({ label, value, sub, color, icon }: { label: string; value: string | number; sub?: string; color?: string; icon?: string }) {
  ...
  return (
    <div style={{
      ...
      padding: "20px 22px", borderLeft: `4px solid ${c}`,
      animation: "fadeInUp 0.35s ease both",
    }}>
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
        <div style={{ minWidth: 0 }}>
          <div style={{ fontSize: 11, fontWeight: 600, color: theme.textMuted, textTransform: "uppercase", letterSpacing: "0.6px", marginBottom: 8 }}>{label}</div>
          <div style={{ fontSize: 26, fontWeight: 800, color: theme.textPrimary, letterSpacing: "-0.6px", lineHeight: 1 }}>{value}</div>
          {sub && <div style={{ fontSize: 12, color: theme.textMuted, marginTop: 5 }}>{sub}</div>}
        </div>
        {icon && (
          <div style={{ width: 40, height: 40, borderRadius: 10, background: `${c}18`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, flexShrink: 0 }}>{icon}</div>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div style={{ animation: "fadeInUp 0.3s ease" }}>
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ fontSize: 22, fontWeight: 800, color: theme.textPrimary, letterSpacing: "-0.5px" }}>Probate Dashboard</h1>
        <p style={{ fontSize: 13, color: theme.textSecondary, marginTop: 4 }}>Real-time overview of all deceased account estate settlements</p>
      </div>

      {/* KPI row — auto-fit so cards always fill the row */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 16, marginBottom: 24 }}>
        ...
        <KpiCard label="Total Estate Value" value={`RM ${totalRm.toLocaleString("en-MY", { minimumFractionDigits: 2 })}`} icon="💰" color="#8B5CF6" />
      </div>

      {/* Recent estates table — full width */}
      <GlassCard
        title="Recent Estates"
        subtitle="Latest probate cases across all statuses"
        action={
          <Link to="/estates" style={{ fontSize: 12, fontWeight: 600, color: theme.accent, display: "flex", alignItems: "center", gap: 4 }}>
            View all →
          </Link>
        }
      >
              ...
              {loading
                ...
                ? (
                  <tr><td colSpan={5} style={{ padding: "40px 16px", textAlign: "center" }}>
                    ...
                    <Link to="/estates" style={{ fontSize: 13, color: theme.accent, fontWeight: 600 }}>Create your first estate →</Link>
                  </td></tr>
                )
                : estates.slice(0, 10).map((e: any) => (
                  <tr key={e.id} onClick={() => window.location.href = `/estates/${e.id}`}
                    ...
                    <td style={{ padding: "13px 16px" }}>
                      <StatusBadge status={e.status} pulse={["SCANNING", "DISTRIBUTING", "EXECUTING"].includes(e.status)} />
                    </td>
                    ...
                    <td style={{ padding: "13px 16px" }}>
                      <span style={{ fontSize: 12, fontWeight: 600, background: theme.infoLight, color: theme.info, padding: "2px 8px", borderRadius: 12, border: `1px solid ${theme.accentLight}` }}>{e.beneficiary_count}</span>
                    </td>
      ...
      {/* Quick-start — only shown when empty, fills full width */}
        ...
        <div style={{ marginTop: 20, display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 14 }}>
          {[
            ...
            <GlassCard key={s.step} style={{ borderTop: `3px solid ${s.color}` }}>
              <div style={{ width: 28, height: 28, borderRadius: 8, background: `${s.color}18`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 800, color: s.color, marginBottom: 10 }}>{s.step}</div>
